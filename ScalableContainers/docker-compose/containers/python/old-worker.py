...
#    print("Predicted to be class ", pred, "with probability", prob)
    if (logToDB == 1):
...
pgconn = psycopg2.connect(user="postgres", password="password", 
                          host="192.168.1.37", port="5432", database="postgres")

connection = pika.BlockingConnection(pika.ConnectionParameters(host='192.168.1.37'))
